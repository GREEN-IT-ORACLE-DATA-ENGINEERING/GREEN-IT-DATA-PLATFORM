...
L'objectif de cette mission était de mettre en place un système de sauvegarde fiable et automatisé pour la Pluggable Database GREEN_IT_PDB, garantissant la sécurité des données , la continuité d'activité , et la réduction des interventions manuelles .

# Connexion et Préparation de la Base

J'ai accédé à l'environnement Oracle sur machine virtuelle , assurant un contrôle total de la base de données.

La base a été préparée pour les sauvegardes , en s'assurant qu'aucune session n'interfère avec le processus et que les journaux de transactions sont prêts à être archivés.
